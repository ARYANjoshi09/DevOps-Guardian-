...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetchIncidents();
    fetchAnalytics();
    const incidentInterval = setInterval(fetchIncidents, 2000);
    const analyticsInterval = setInterval(fetchAnalytics, 10000); // Update analytics every 10s
    return () => {
      ...
      <header className="flex items-center justify-between mb-8 border-b border-zinc-800 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm">Projects & Incidents</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/50 mt-auto">
            ...
            <div className="space-y-4">
              <StatRow label="Total Incidents" value={analytics.totalIncidents.toString()} />
              <StatRow
                label="Resolved"
                value={`${analytics.resolvedIncidents}/${analytics.totalIncidents}`}
              />
              <StatRow
                label="Mean Time to Resolve"
                value={analytics.avgMTTR > 0 ? `${analytics.avgMTTR}s` : "N/A"}
              />
              <StatRow label="Success Rate" value={`${analytics.successRate}%`} />
            </div>
          </div>
        </div>
